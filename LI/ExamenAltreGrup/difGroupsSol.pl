% Every year, students of Plesanton high-school are distributed into
...
% distribution.
%
% COMPULSORY: Use variables st-S-T: student S is in team T
...
incompatible(S1,S2):-
    ...
    lastYearTeam(T), member(S1,T), member(S2,T).
...
cadaSunG:- student(S), findall(st-S-T,team(T),L), exactly(1,L),fail.
cadaSunG.

nomesAmbUn:-  team(T),incompatible(S1,S2),incompatible(S1,S3), S2 \= S3, 
		writeClause([\+st-S1-T,\+st-S2-T,\+st-S3-T]), fail. 
nomesAmbUn.

numSpG:- teamSize(TS),team(T),findall(st-S-T, student(S),L), exactly(TS,L),fail.
numSpG. 
