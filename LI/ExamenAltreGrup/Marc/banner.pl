...
:-include(inputExamples/input5). % Load input
:-dynamic(varNumber/3).
...
pieceInCell(P,Sx,Sy,X,Y,0):- piece(P), pieceSize(P,W,H),cell(Sx,Sy), Mx is Sx+W-1, My is Sy+H-1,between(Sx,Mx,X), between(Sy,My,Y), cell(X,Y).  %no rotated
pieceInCell(P,Sx,Sy,X,Y,1):- piece(P), pieceSize(P,H,W),cell(Sx,Sy), Mx is Sx+W-1, My is Sy+H-1,between(Sx,Mx,X), between(Sy,My,Y), cell(X,Y). %rotated
...
cellPiecesIfRotated:-
	piece(P),
	cell(Sx,Sy),
	findall(pieceCell-P-X-Y, pieceInCell(P,Sx,Sy,X,Y,1),Lits),
	append([\+pieceStarts-P-Sx-Sy],Lits,L1),
	append([\+rotated-P],L1,L),
	writeClause(L),
	fail.
cellPiecesIfRotated.
...
cellPiecesIfNoRotated:-
	piece(P),
	cell(Sx,Sy),
	findall(pieceCell-P-X-Y, pieceInCell(P,Sx,Sy,X,Y,0),Lits),
	append([\+pieceStarts-P-Sx-Sy],Lits,L1),
	append([rotated-P],L1,L),
	writeClause(L),
	fail.
...
pieceCellsAreSize:-
	...
	pieceSize(P,H,W),
	S is W*H,
	findall(pieceCell-P-X-Y,cell(X,Y),Lits),
	exactly(S,Lits),
	fail.
pieceCellsAreSize.	
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% DISPLAYSOL:
...
treatResult(10,_):- %   shell('cat model',_),	
    ...
    shell('cat header clauses > infile.cnf',_),
    write('Launching picosat...'), nl,
	shell('./solvers/picosat-960/picosat -v -o model infile.cnf', Result),  % if sat: Result=10; if unsat: Result=20.
