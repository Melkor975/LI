...
workingHourForTypeAndStartH(1,StartH,H):- between(6,7,I), H is (StartH+I) mod 24. %Type=2: WWWRRWWW
...
writeClauses:-
	%exactlyOneTypePerNurse,
%%	exactlyOneStartPerNurse,
%	nurseWorksExactly6Hours,
%	atLeastNededPerHour,
%	typeWorksStart,
	true.
...
validHora(N,H):-
	hour(H),
	nurseIDandBlocking(N,H1,H2),
	\+hourInRange(H1,H2,H).
	

nurseWorksExactly6Hours:-
	nurse(N),
	findall(worksNH-N-H, hour(H), Lits),
	exactly(6,Lits),
	fail.
nurseWorksExactly6Hours.
...
typeWorksStart:-
	nurse(N),
	hour(S),
	workingHourForTypeAndStartH(1, S, H),
	validHora(N,H),
	writeClause([worksNH-N-H,\+startsNH-N-S]),
	fail.
typeWorksStart.
...
atLeastNededPerHour:- 
	needed(Hour, Need),
	findall(worksNH-N-Hour, nurse(N), Lits),
	atLeast(Need, Lits),
	fail.
atLeastNededPerHour.
